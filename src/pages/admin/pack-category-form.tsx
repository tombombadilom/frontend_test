...
interface PackCategoryFormProps {
  editMode?: boolean;
}

export function PackCategoryForm({ editMode = false }: PackCategoryFormProps) {
  const navigate = useNavigate();
  const { id: urlId } = useParams<{ id: string }>();
  const [initialData, setInitialData] = useState<PackCategory | null>(null);
      ...
      if (editMode && urlId) {
        try {
          ...
          if (category) {
            ...
            navigate('/admin/pack-categories');
          }
        } catch (error) {
  ...
  const onSubmit = async (data: CategoryFormValues) => {
    try {
      const categoryData: PackCategory = {
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="flex justify-end space-x-4">
            <Button
              type="button"
              variant="outline"
              onClick={() => navigate('/admin/pack-categories')}
            >
              Annuler
            </Button>
